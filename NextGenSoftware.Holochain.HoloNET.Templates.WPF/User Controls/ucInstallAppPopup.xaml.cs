...
namespace NextGenSoftware.Holochain.HoloNET.Templates.WPF.UserControls
{
    ...
    public partial class ucInstallAppPopup : UserControl
    {
        ...
        private void btnPopupInstallhAppOk_Click(object sender, RoutedEventArgs e)
        {
            ...
            else if (InputTextBox.Text == HoloNETManager.Instance.HoloNETEntryDemoAppId)
            {
                lblInstallAppErrorMessage.Text = "Sorry that name is reserved for the HoloNETEntry (Inernal Connection) popup.";
                lblInstallAppErrorMessage.Visibility = Visibility.Visible;
            }
            else if (InputTextBox.Text == HoloNETManager.Instance.HoloNETCollectionDemoAppId)
            {
            ...
            else
            {
                ...
                HoloNETManager.Instance.ShowStatusMessage($"Generating New AgentPubKey...", StatusMessageType.Information, true,  ucHoloNETEntry);
                ...
                Dispatcher.InvokeAsync(async () =>
                {
                    ...
                    if (agentPubKeyResult != null && !agentPubKeyResult.IsError)
                    {
                        ...
                        HoloNETManager.Instance.ShowStatusMessage($"AgentPubKey Generated Successfully. AgentPubKey: {agentPubKeyResult.AgentPubKey}", StatusMessageType.Success, false, ucHoloNETEntry);

                        HoloNETManager.Instance.LogMessage($"ADMIN: Installing hApp {InputTextBox.Text} ({HoloNETManager.Instance.InstallinghAppPath})...");
                        HoloNETManager.Instance.ShowStatusMessage($"Installing hApp {InputTextBox.Text}...", StatusMessageType.Information, true);

                        //We can use async or non-async versions for every function.
                        HoloNETManager.Instance.HoloNETClientAdmin.AdminInstallApp(HoloNETManager.Instance.HoloNETClientAdmin.HoloNETDNA.AgentPubKey, InputTextBox.Text, HoloNETManager.Instance.InstallinghAppPath);
                    }
                });
            }
        }
